...
export const CurrencyProvider: React.FC<CurrencyProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    localStorage.setItem('starredCurrencies', JSON.stringify(starredCurrencies));
  }, [starredCurrencies]);
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        ...
        const currencyArray = Object.entries(data.currencies).map(([code, description]: [string, string]) => ({
          code,
          name: description
        }));
